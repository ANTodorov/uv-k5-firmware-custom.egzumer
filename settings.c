...
void SETTINGS_InitEEPROM(void)
{
	...
	// 0E70..0E77
	EEPROM_ReadBuffer(EEPROM_SETTINGS_OFF, Data._8, 16);
#ifdef ENABLE_ALT_CHANNELS
	gEeprom.CHAN_1_CALL          = IS_MR_CHANNEL(Data._16[0]) ? Data._16[0] : MR_CHANNEL_FIRST;
	gEeprom.SQUELCH_LEVEL        = (Data._16[1] < 10) ? Data._16[1] : 1;
	gEeprom.TX_TIMEOUT_TIMER     = (Data._16[2] < 11) ? Data._16[2] : 1;
	#ifdef ENABLE_NOAA
		gEeprom.NOAA_AUTO_SCAN   = (Data._16[3] <  2) ? Data._16[3] : false;
	#endif
	gEeprom.KEY_LOCK             = (Data._16[4] <  2) ? Data._16[4] : false;
	#ifdef ENABLE_VOX
		gEeprom.VOX_SWITCH       = (Data._16[5] <  2) ? Data._16[6] : false;
		gEeprom.VOX_LEVEL        = (Data._16[6] < 10) ? Data._16[7] : 1;
	#endif
	gEeprom.MIC_SENSITIVITY      = (Data._16[7] <  5) ? Data._16[8] : 4;
#else
...
#endif
	// 0E78..0E7F
	EEPROM_ReadBuffer(0x0E78, Data._8, 8);
	...
	// 0E80..0E87
	EEPROM_ReadBuffer(EEEPROM_DISP_CH_STORE_OFF, Data._8, 16);
#ifdef ENABLE_ALT_CHANNELS
	gEeprom.ScreenChannel[0]   = IS_VALID_CHANNEL(Data._16[0]) ? Data._16[0] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	gEeprom.ScreenChannel[1]   = IS_VALID_CHANNEL(Data._16[3]) ? Data._16[3] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	...
	gEeprom.FreqChannel[0]     = IS_FREQ_CHANNEL(Data._16[2])  ? Data._16[2] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	gEeprom.FreqChannel[1]     = IS_FREQ_CHANNEL(Data._16[5])  ? Data._16[5] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
#ifdef ENABLE_NOAA
...
#endif
#else
	gEeprom.ScreenChannel[0]   = IS_VALID_CHANNEL(Data._8[0]) ? Data._8[0] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	gEeprom.ScreenChannel[1]   = IS_VALID_CHANNEL(Data._8[3]) ? Data._8[3] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	...
	gEeprom.FreqChannel[0]     = IS_FREQ_CHANNEL(Data._8[2])  ? Data._8[2] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
	gEeprom.FreqChannel[1]     = IS_FREQ_CHANNEL(Data._8[5])  ? Data._8[5] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
#ifdef ENABLE_NOAA
	...
	// 0F18..0F1F
	EEPROM_ReadBuffer(EEPROM_SCANLIST_OFF, Data._8, 16);
#ifdef ENABLE_ALT_CHANNELS
	gEeprom.SCAN_LIST_DEFAULT = (Data._16[0] < 3) ? Data._16[0] : 0;  // we now have 'all' channel scan option
	for (unsigned int i = 0; i < 2; i++)
	{
		const unsigned int j = 1 + (i * 3);
		gEeprom.SCAN_LIST_ENABLED[i]     = (Data._16[j + 0] < 2) ? Data._16[j] : false;
		gEeprom.SCANLIST_PRIORITY_CH1[i] =  Data._16[j + 1];
		gEeprom.SCANLIST_PRIORITY_CH2[i] =  Data._16[j + 2];
	}
#else
	...
	for( uint16_t i = 0 ; i < ALL_CHANNELS_COUNT / _CHUNKSIZE; i++) {
		EEPROM_ReadBuffer(EEPROM_MR_CH_ATTR_OFF + channel, &gMR_ChannelAttributes[channel], _CHUNKSIZE);
		channel += _CHUNKSIZE;
	}
	if (ALL_CHANNELS_COUNT - channel > 0){
		EEPROM_ReadBuffer(EEPROM_MR_CH_ATTR_OFF + channel, &gMR_ChannelAttributes[channel], ALL_CHANNELS_COUNT - channel);
	}
#endif
...
uint32_t SETTINGS_FetchChannelFrequency(const int channel)
{
	...
	EEPROM_ReadBuffer(EEPROM_MR_CH_FREQ_OFF + channel * 16, &info, sizeof(info));
...
void SETTINGS_FetchChannelName(char *s, const int channel)
{
	...
	EEPROM_ReadBuffer(EEPROM_MR_CH_NAME_OFF + channel * 16, s, 10);
...
void SETTINGS_SaveVfoIndices(void)
{
	...
	#ifndef ENABLE_NOAA
		EEPROM_ReadBuffer(EEEPROM_DISP_CH_STORE_OFF, State, sizeof(State));
	#endif
	...
	EEPROM_WriteBuffer(EEEPROM_DISP_CH_STORE_OFF, State);
}
...
void SETTINGS_SaveSettings(void)
{
...
#ifdef ENABLE_ALT_CHANNELS
	uint16_t  State2[8];
	State2[0] = gEeprom.CHAN_1_CALL;
	State2[1] = gEeprom.SQUELCH_LEVEL;
	State2[2] = gEeprom.TX_TIMEOUT_TIMER;
	#ifdef ENABLE_NOAA
		State2[3] = gEeprom.NOAA_AUTO_SCAN;
	#else
		State2[3] = false;
	#endif
	State2[4] = gEeprom.KEY_LOCK;
	#ifdef ENABLE_VOX
		State2[5] = gEeprom.VOX_SWITCH;
		State2[6] = gEeprom.VOX_LEVEL;
	#else
		State2[5] = false;
		State2[6] = 0;
	#endif
	State2[7] = gEeprom.MIC_SENSITIVITY;
	EEPROM_WriteBuffer(EEPROM_SETTINGS_OFF, State2);
#else
	...
	EEPROM_WriteBuffer(EEPROM_SETTINGS_OFF, State);
#endif
...
#ifdef ENABLE_ALT_CHANNELS
	State2[0] = gEeprom.SCAN_LIST_DEFAULT;
	State2[1] = gEeprom.SCAN_LIST_ENABLED[0];
	State2[2] = gEeprom.SCANLIST_PRIORITY_CH1[0];
	State2[3] = gEeprom.SCANLIST_PRIORITY_CH2[0];
	State2[4] = gEeprom.SCAN_LIST_ENABLED[1];
	State2[5] = gEeprom.SCANLIST_PRIORITY_CH1[1];
	State2[6] = gEeprom.SCANLIST_PRIORITY_CH2[1];
	State2[7] = 0xFFFF;
	EEPROM_WriteBuffer(EEPROM_SCANLIST_OFF, State2);
#else
	...
	State[7] = 0xFF;
	EEPROM_WriteBuffer(EEPROM_SCANLIST_OFF, State);
#endif
...
void SETTINGS_SaveChannel(channel_t Channel, uint8_t VFO, const VFO_Info_t *pVFO, uint8_t Mode)
{
	...
	uint16_t OffsetVFO = EEPROM_MR_CH_FREQ_OFF + Channel * 16;

	if (IS_FREQ_CHANNEL(Channel)) { // it's a VFO, not a channel
		OffsetVFO  = (VFO == 0) ? EEPROM_FREQ_CH_FREQ_OFF : (EEPROM_FREQ_CH_FREQ_OFF + 16);
...
void SETTINGS_SaveChannelName(channel_t channel, const char * name)
{
	...
	EEPROM_WriteBuffer(EEPROM_MR_CH_NAME_OFF + offset, buf);
	EEPROM_WriteBuffer(EEPROM_MR_CH_NAME_OFF + 8 + offset, buf + 8);
}
...
void SETTINGS_UpdateChannel(channel_t channel, const VFO_Info_t *pVFO, bool keep)
{
...
#endif
	{
		...
		uint16_t offset = EEPROM_MR_CH_ATTR_OFF + (channel & ~7u);
